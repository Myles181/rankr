...
exports.default = FanDashboard;
const react_1 = require("react");
const API = 'http://localhost:4000';
function FanDashboard() {
    const [session, setSession] = (0, react_1.useState)(null);
    const [pools, setPools] = (0, react_1.useState)([]);
    const [currentPoolId, setCurrentPoolId] = (0, react_1.useState)(null);
    const [leaderboard, setLeaderboard] = (0, react_1.useState)([]);
    const [myRankInfo, setMyRankInfo] = (0, react_1.useState)({ rank: null, totalFans: 0, formatted: '0m' });
    (0, react_1.useEffect)(() => {
    ...
    const handleJoinPool = (poolId, poolName) => {
    ...
    const handleSync = () => {
        if (!currentPoolId)
            return alert('Join a pool first to sync listening time.');
        setMyRankInfo(prev => ({
            ...prev,
            formatted: '3h 15m',
            rank: 2
        ...
        if (newLb[1])
            newLb[1].displayName = session?.displayName || 'Mock Fan';
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          <div className="pools-grid" id="poolsGrid">
            {pools.length === 0 ? (<div style={{ color: 'var(--muted)', fontSize: '.875rem', padding: '1rem 0' }}>Loading pools...</div>) : (pools.map(pool => {
            const ends = new Date(pool.endsAt);
            const daysLeft = Math.ceil((ends.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
            return (<div className="pool-card" key={pool.id}>
          ...
          <div className="lb-card">
            ...
            <div id="leaderboard">
              {leaderboard.length === 0 ? (<div style={{ padding: '2rem', textAlign: 'center', color: 'var(--muted)', fontSize: '.875rem' }}>
                  Join a pool to see rankings
                </div>) : (leaderboard.map((fan) => {
            ...
            return (<div className={`lb-row ${isMe ? 'is-me' : ''}`} key={fan.rank}>
...
//# sourceMappingURL=FanDashboard.js.map